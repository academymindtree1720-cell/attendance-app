...
export async function getEmployees() {
  ...
  return rows
    .filter((row) => {
      ...
      if (lowerEmail === "email" || lowerEmail === "admin@" || lowerEmail === "") return false;
      return true;
    })
...
export async function getAttendanceRecords() {
  ...
  return rows
    .filter((row) => {
      ...
      const [date, email, name, type, time] = row;
      if (!date || !email || !name || !type || !time) return false;
      if (String(type).toLowerCase() === "type") return false;
      return true;
    })
    .map(([date, email, name, type, time, note]) => ({
